...
export default function AdminPage() {
  ...
  const fetchReports = async () => {
    try {
      const response = await axios.get(`${baseUrl}/activity/list-reports/`);
...
function ReportTable({reports, setReports}){
  ...
  const handleCreate = async () => {
    try {
      const response = await axios.post(`${baseUrl}/activity/create-report/`, { description });
      if (response.status === 201) {
        console.log("Report successfully created");
        // Optionally, you can refetch the reports or update the state to include the new report
        const response = await axios.get(`${baseUrl}/activity/list-reports/`);
        setReports(response.data);
        console.log(reports)
      }
  ...
  const handleView = async (reportId) => {
    ...
    try {
      const categoryResponse = await axios.get(`${baseUrl}/activity/view-category-reports/${reportId}`);
      const businessResponse = await axios.get(`${baseUrl}/activity/view-business-reports/${reportId}`);
  ...
  return(
    <>
      <Button variant="primary" onClick={() => setShowCreateModal(true)}>Create Report</Button>
      <Table striped bordered hover>
        <thead>
          <tr>
            <th>Admin Name</th>
        ...
        <tbody>
          {reports.map((report, index) => (
            <tr key={index}>
              ...
              <td>
                <div>
                  <Button variant="outline-primary" onClick={()=>{handleView(report.report_id)}}>View</Button>
      ...
      <Modal show={showModal} onHide={handleClose} size="lg" centered>
        ...
        <Modal.Body>
          <Dashboard categoryData={categoryData} businessData={businessData}/>
        </Modal.Body>
      </Modal>
      <Modal show={showCreateModal} onHide={handleCreateClose} size="lg" centered>
        <Modal.Header closeButton>
